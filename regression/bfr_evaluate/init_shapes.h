...
initShapes(std::vector<ShapeDesc> & shapes) {
    ...
    //  Shapes currently "unsupported" by new topological constructions -- the
    //  ManifoldFaceBuilder currently does not handle the self-overlaps that
    //  occur with really low interior valence (2 for Catmark, 3 for Loop).  We
    //  could use the General descriptor/builder here, but ultimately we need
    //  the Manifold descriptor to be more robust.
    //
    //  These are commented out both here and in the list below:
    /*
