...
int
countFVarValuesAtVertex(Far::TopologyLevel const & baseLevel, Index vIndex) {
    ...
    for (int i = 0; i < vFaces.size(); ++i) {
        Index fvarValue = baseLevel.GetFaceFVarValues(vFaces[i])[vInFace[i]];
...
TestMesh::SharpenFromUvChannel(TestMesh const & srcMesh, FVarOption option) {
    ...
    case Sdc::Options::FVAR_LINEAR_CORNERS_PLUS2:
        ...
        for (int i = 0; i < srcContainer.vertexCount; ++i) {
            ...
            int fvarValuesAtVertex = countFVarValuesAtVertex(srcBaseLevel, i);
...
bool
TestMesh::WriteToObjFile(char const * objFilename) const {

printf("... writing to Obj file '%s' ...\n", objFilename);

    FILE * fptr = fopen(objFilename, "w");
    if (fptr == 0) return false;

    TopologyLevel const & lastLevel = _topRefiner->GetLevel(_topRefiner->GetMaxLevel());

    int nFaces = lastLevel.GetNumFaces();
    int nVerts = lastLevel.GetNumVertices();
    int nUvs   = HasUvs() ? lastLevel.GetNumFVarValues(GetUvChannel()) : 0;

    Coord3 const * xyzCoordsLast =        &_xyzCoords[_topRefiner->GetNumVerticesTotal() - nVerts];
    Coord3 const * uvwCoordsLast = nUvs ? &_uvwCoords[_topRefiner->GetNumFVarValuesTotal(GetUvChannel()) - nUvs] : 0;

    //  Print vertex positions
    for (int i = 0; i < nVerts; ++i) {
        Coord3 const & xyz = xyzCoordsLast[i];
        fprintf(fptr, "v %f %f %f\n", xyz.x, xyz.y, xyz.z);
    }
        
    //  Print UV values (only the UV of the stored UVW)
    for (int i = 0; i < nUvs; ++i) {
        Coord3 const & uvw = uvwCoordsLast[i];
        fprintf(fptr, "vt %f %f\n", uvw.x, uvw.y);
    }

    //  Print faces
    for (int i = 0; i < nFaces; ++i) {
        ConstIndexArray fVerts = lastLevel.GetFaceVertices(i);
        ConstIndexArray fUvs   = nUvs ? lastLevel.GetFaceFVarValues(i, GetUvChannel()) : fVerts;
